...
class NewsInfoCell: UITableViewCell, NewsCellProtocol {
	...
	func setCell(info: Novelty) {
		...
		var fullAuthor = "POR "
		fullAuthor.append(info.autores![0].uppercased())
		let nameForColor = info.autores![0].uppercased()
		...
		self.buildIconText(label: self.datetime, fontSize: 12.0, text: info.atualizadoEm!.dateOutput(to: "dd/MM/yy HH:mm"), fontName: "clocko")
